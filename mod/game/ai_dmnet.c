...
int BotGetItemLongTermGoal(bot_state_t *bs, int tfl, bot_goal_t *goal)
{
	...
	// See if we should bandage
	// bot has not seen the enemy in the last 3 seconds or is limping
	if((bs->cur_ps.stats[STAT_UTPMOVE_FLAGS] & UT_PMF_LIMPING) || (bs->cur_ps.stats[STAT_UTPMOVE_FLAGS] & UT_PMF_BLEEDING))
	{
		if((bs->enemyvisible_time < FloatTime() - 3.0f))
		{
			if((FloatTime() - bs->last_bandage_time) > 1.0f)
			{
				trap_EA_Action(bs->client, ACTION_BANDAGE );
				bs->last_bandage_time = FloatTime();
			}
		}
	}
	...
	if (bs->ltg_time < FloatTime())
	{
...

		{
			...
			goalEnt = level.LTGItems[BotFindSpawnToPatrol(bs)];

			if (goalEnt->r.absmin[0] || goalEnt->r.absmin[1] || goalEnt->r.absmin[2])
			{
				new_goal.origin[0] = goalEnt->r.absmin[0] + (goalEnt->r.absmax[0] - goalEnt->r.absmin[0]) / 2;
				new_goal.origin[1] = goalEnt->r.absmin[1] + (goalEnt->r.absmax[1] - goalEnt->r.absmin[1]) / 2;
				new_goal.origin[2] = goalEnt->r.absmin[2] + (goalEnt->r.absmax[2] - goalEnt->r.absmin[2]) / 2;
			}
			else
			{
				VectorCopy(goalEnt->s.origin, new_goal.origin);
			}

			areanum = BotPointAreaNum(new_goal.origin);

			if (areanum)
			{
				VectorCopy ( bs->origin, bs->ltgStuckOrigin );
				bs->ltgStuckTime   = FloatTime ( ) + 2;

				new_goal.areanum   = areanum;
				new_goal.mins[0]   = -55;
				new_goal.mins[1]   = -55;
				new_goal.mins[2]   = -85;
				new_goal.maxs[0]   = 55;
				new_goal.maxs[1]   = 55;
				new_goal.maxs[2]   = 85;
				new_goal.flags	   = GFL_ITEM | GFL_AIR;
				new_goal.number    = 0;
				new_goal.iteminfo  = 0;
				new_goal.entitynum = 0;

				bs->ltg_time	   = FloatTime() + 40;

				trap_BotPushGoal(bs->gs, &new_goal);
			}
			else
			{
				//
	...
	else if (bs->ltgStuckTime < FloatTime())
	{
		VectorSubtract ( bs->origin, bs->ltgStuckOrigin, bs->ltgStuckOrigin );

		if (VectorLengthSquared ( bs->ltgStuckOrigin ) < 100 * 100)
		{
			bs->ltg_time = 0;
		}
...
{
	if ((bs->inventory[INVENTORY_BERETTA] > 0) && (bs->inventory[INVENTORY_BERETTA_AMMO] > 0))
	{
		return WEAPONINDEX_BERETTA;
	}
	else if (bs->inventory[INVENTORY_DEAGLE] > 0 && bs->inventory[INVENTORY_DEAGLE_AMMO] > 0)
	{
		return WEAPONINDEX_DEAGLE;
	}
	else if (bs->inventory[INVENTORY_SPAS12] > 0 && bs->inventory[INVENTORY_SPAS12_AMMO] > 0)
	{
		return WEAPONINDEX_SPAS12;
	}
	/*else if (bs->inventory[INVENTORY_BENELLI] > 0 && bs->inventory[INVENTORY_BENELLI_AMMO] > 0)
	{
		return WEAPONINDEX_BENELLI;
	}*/
	else if (bs->inventory[INVENTORY_MP5K] > 0 && bs->inventory[INVENTORY_MP5K_AMMO] > 0)
	{
		return WEAPONINDEX_MP5K;
	}
	/*else if (bs->inventory[INVENTORY_P90] > 0 && bs->inventory[INVENTORY_P90_AMMO] > 0)
	{
		return WEAPONINDEX_P90;
	}*/
	else if (bs->inventory[INVENTORY_UMP45] > 0 && bs->inventory[INVENTORY_UMP45_AMMO] > 0)
	{
		return WEAPONINDEX_UMP45;
	}
	else if (bs->inventory[INVENTORY_LR] > 0 && bs->inventory[INVENTORY_LR_AMMO] > 0)
	{
		return WEAPONINDEX_LR;
	}
	else if (bs->inventory[INVENTORY_G36] > 0 && bs->inventory[INVENTORY_G36_AMMO] > 0)
	{
		return WEAPONINDEX_G36;
	}
	else if (bs->inventory[INVENTORY_HK69] > 0 && bs->inventory[INVENTORY_HK69_AMMO] > 0)
	{
		return WEAPONINDEX_HK69;
	}
	else if (bs->inventory[INVENTORY_PSG1] > 0 && bs->inventory[INVENTORY_PSG1_AMMO] > 0)
	{
		return WEAPONINDEX_PSG1;
	}
	else if (bs->inventory[INVENTORY_SR8] > 0 && bs->inventory[INVENTORY_SR8_AMMO] > 0)
	{
		return WEAPONINDEX_SR8;
	}
	else if (bs->inventory[INVENTORY_AK103] > 0 && bs->inventory[INVENTORY_AK103_AMMO] > 0)
	{
		return WEAPONINDEX_AK103;
	}
	else if (bs->inventory[INVENTORY_M4] > 0 && bs->inventory[INVENTORY_M4_AMMO] > 0)
	{
		return WEAPONINDEX_M4;
	}
	else if (bs->inventory[INVENTORY_NEGEV] > 0 && bs->inventory[INVENTORY_NEGEV_AMMO] > 0)
	{
		return WEAPONINDEX_M4;
	}
	else if ((bs->inventory[INVENTORY_GLOCK] > 0) && (bs->inventory[INVENTORY_GLOCK_AMMO] > 0))
	{
		return WEAPONINDEX_GLOCK;
	}
	else if (bs->inventory[INVENTORY_COLT1911] > 0 && bs->inventory[INVENTORY_COLT1911_AMMO] > 0)
	{
       return WEAPONINDEX_COLT1911;
    }
	else if (bs->inventory[INVENTORY_MAC11] > 0 && bs->inventory[INVENTORY_MAC11_AMMO] > 0)
	{
        return WEAPONINDEX_MAC11;
    }
	/*else if ((bs->inventory[INVENTORY_MAGNUM] > 0) && (bs->inventory[INVENTORY_MAGNUM_AMMO] > 0))
	{
		return WEAPONINDEX_MAGNUM;
	}*/
/*
	else if ((bs->inventory[INVENTORY_DUAL_BERETTA] > 0) && (bs->inventory[INVENTORY_BERETTA_AMMO] > 0))
	{
		return WEAPONINDEX_DUAL_BERETTA;
	}
	else if (bs->inventory[INVENTORY_DUAL_DEAGLE] > 0 && bs->inventory[INVENTORY_DEAGLE_AMMO] > 0)
	{
		return WEAPONINDEX_DUAL_DEAGLE;
	}
	else if ((bs->inventory[INVENTORY_DUAL_GLOCK] > 0) && (bs->inventory[INVENTORY_GLOCK_AMMO] > 0))
	{
		return WEAPONINDEX_DUAL_GLOCK;
	}
	else if ((bs->inventory[INVENTORY_DUAL_MAGNUM] > 0) && (bs->inventory[INVENTORY_MAGNUM_AMMO] > 0))
	{
		return WEAPONINDEX_DUAL_MAGNUM;
	}
*/
	else
	{
...
void BotClearPath(bot_state_t *bs, bot_moveresult_t *moveresult)
{
	...
	if (bs->kamikazebody)
	{
		...
		if (!(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)))
		{
			...
			if (moveresult->weapon)
			{
				...
				if (UT_WEAPON_GETID(bs->cur_ps.weaponData[bs->cur_ps.weaponslot]) == moveresult->weapon)
				{
		...
		!(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)))
	{
		...
		if (bestmine != -1)
		{
			...
			if ((bs->inventory[INVENTORY_HK69] > 0) && (bs->inventory[INVENTORY_HK69_AMMO] > 0))
			{
...
int AINode_Seek_ActivateEntity(bot_state_t *bs)
{
	...
	if (bs->activatestack->shoot)
	{
		...
		if ((bsptrace.fraction >= 1.0) || (bsptrace.ent == goal->entitynum))
		{
			...
			// if holding the right weapon
			if (UT_WEAPON_GETID(bs->cur_ps.weaponData[bs->cur_ps.weaponslot]) == bs->activatestack->weapon)
			{
...
//////////////////////////////////////////////////////////////////////////////////////////
// Name 	  : G_InitUTBotItemGoals
// Description: Intializes urban terror bots long term item goals, the default ltg
// Author	  : GottaBeKD
///////////////////////////////////////////////////////////////////////////////////////////
void G_InitUTBotItemGoals ( void )
{
	char	   *groups[MAX_BOT_ITEM_GOALS];

	gentity_t  *spot	 = NULL;
	int    numGroups = 0;
	int    i;

	// Look through the ut_spawns and add them
	while ((spot = G_FindClassHash (spot, HSH_info_ut_spawn)) != NULL)
	{
		// Not really a spawn, hmm.
		if(!spot->spawn)
		{
			continue;
		}
		...
		{
			if(!Q_stricmp(spot->spawn->group, groups[i]))
			{
				break;
			}
		}

		// If it was in the list then just continue on
		if(i < numGroups)
		{
			continue;
		}

		// If we still have room for more bot goals then
		// add it, otherwise just give up
		...
		// Add the group
		groups[numGroups++] = spot->spawn->group;
	}

	// GottaBeKD: Added BUILD19
	// Look through the info_player_deathmatch spawns and add them
	spot = NULL;

	while ((spot = G_FindClassHash (spot, HSH_func_rotating_door)) != NULL)
	{
		if(level.numLTGItems < MAX_BOT_ITEM_GOALS)
		{
			level.LTGItems[level.numLTGItems++] = spot;
	...
	while ((spot = G_FindClassHash (spot, HSH_func_door)) != NULL)
	{
		if(level.numLTGItems < MAX_BOT_ITEM_GOALS)
		{
			level.LTGItems[level.numLTGItems++] = spot;
	...
	while ((spot = G_FindClassHash (spot, HSH_team_CTF_redspawn)) != NULL)
	{
		if(level.numLTGItems < MAX_BOT_ITEM_GOALS)
		{
			level.LTGItems[level.numLTGItems++] = spot;
	...
	while ((spot = G_FindClassHash (spot, HSH_team_CTF_bluespawn)) != NULL)
	{
		if(level.numLTGItems < MAX_BOT_ITEM_GOALS)
		{
			level.LTGItems[level.numLTGItems++] = spot;
...
//////////////////////////////////////////////////////////////////////////////////////////
// Name 	  : BotFindSpawnToPatrol
// Description: Bot Default Goal Finder
// Author	  : GottaBeKD
///////////////////////////////////////////////////////////////////////////////////////////
int BotFindSpawnToPatrol( bot_state_t *bs )
{
	...
	// HACK:  This is a super hack to keep bots moving a bit when they
	//	  cant find their way out.
	if (bs->harvestaway_time)
	{
	...
	for(count = 0; count < 5; count++)
	{
		...
		// pick a spot
		randomIndex = (rand() % level.numLTGItems) ;
		goalEnt 	= level.LTGItems[randomIndex];

		if (goalEnt->r.absmin[0] || goalEnt->r.absmin[1] || goalEnt->r.absmin[2])
		{
			dir[0] = goalEnt->r.absmin[0] + (goalEnt->r.absmax[0] - goalEnt->r.absmin[0]) / 2;
			dir[1] = goalEnt->r.absmin[1] + (goalEnt->r.absmax[1] - goalEnt->r.absmin[1]) / 2;
			dir[2] = goalEnt->r.absmin[2] + (goalEnt->r.absmax[2] - goalEnt->r.absmin[2]) / 2;
		}
		else
		{
			VectorCopy(goalEnt->s.origin, dir);
		}

		//distance towards the goal
		VectorSubtract(bs->origin, dir, dir);
		squaredist = VectorLengthSquared(dir);

		if(squaredist > distance)
		{
			break;
		}
	}
