...
void BotChooseWeapon(bot_state_t *bs)
{
	...
	else
	{
		...
		weaponSlot	 = utPSGetWeaponByID ( &bs->cur_ps, newweaponnum );
		weaponData	 = bs->cur_ps.weaponData [weaponSlot];
		...
		if (bs->weaponnum != newweaponnum)
		{
			...
			// Determine how many modes this bot has
			for (modes = 0; bg_weaponlist[newweaponnum].modes[modes].name; modes++)
			{
				;
			}

			// If there are more than one than randomize the mode the bot chooses
			if (modes > 1)
			{
				UT_WEAPON_SETMODE ( g_entities[bs->cur_ps.clientNum].client->ps.weaponData[weaponSlot], (rand() % modes));
			}
		}
		...
		if (UT_WEAPON_GETCLIPS(weaponData) && !UT_WEAPON_GETBULLETS(weaponData) && !(bs->cur_ps.stats[STAT_UTPMOVE_FLAGS] & UT_PMF_RELOADING))
		{
...
void BotUpdateInventory(bot_state_t *bs)
{
	...
	// Clear the inventory so we can rebuild it
	memset(bs->inventory, 0, sizeof(bs->inventory));

	// First add all the weapons to the inventory
	for (weapon = 0; weapon < UT_MAX_WEAPONS; weapon++) //max_weapons
	{
		int  weaponID = UT_WEAPON_GETID ( bs->cur_ps.weaponData[weapon] );

		bs->inventory [bg_weaponlist[weaponID].botInventoryWeaponID] = 1;
		bs->inventory [bg_weaponlist[weaponID].botInventoryAmmoID]	 = UT_WEAPON_GETCLIPS( bs->cur_ps.weaponData[weapon] );
		bs->inventory [bg_weaponlist[weaponID].botInventoryAmmoID]	*= bg_weaponlist[weaponID].bullets;
		bs->inventory [bg_weaponlist[weaponID].botInventoryAmmoID]	+= UT_WEAPON_GETBULLETS( bs->cur_ps.weaponData[weapon] );
	}
	...
	// Flags
	bs->inventory[INVENTORY_REDFLAG]	 = utPSHasItem ( &bs->cur_ps, UT_ITEM_REDFLAG );
	bs->inventory[INVENTORY_BLUEFLAG]	 = utPSHasItem ( &bs->cur_ps, UT_ITEM_REDFLAG );
	bs->inventory[INVENTORY_NEUTRALFLAG] = utPSHasItem ( &bs->cur_ps, UT_ITEM_REDFLAG );
...
float BotAggression(bot_state_t *bs)
{
	...
	if ((bs->inventory[INVENTORY_LR] > 0) && (bs->inventory[INVENTORY_LR_AMMO] > 30))
	{
		return 80;
	}

	if ((bs->inventory[INVENTORY_G36] > 0) && (bs->inventory[INVENTORY_G36_AMMO] > 30))
	{
		return 80;
	}

	if ((bs->inventory[INVENTORY_PSG1] > 0) && (bs->inventory[INVENTORY_PSG1_AMMO] > 6))
	{
		return 90;
	}

	if ((bs->inventory[INVENTORY_SR8] > 0) && (bs->inventory[INVENTORY_SR8_AMMO] > 6))
	{
		return 100;
	}

	if ((bs->inventory[INVENTORY_MP5K] > 0) && (bs->inventory[INVENTORY_MP5K_AMMO] > 30))
	{
		return 70;
	}

	/*if ((bs->inventory[INVENTORY_P90] > 0) && (bs->inventory[INVENTORY_P90_AMMO] > 30))
	{
		return 70;
	}*/

	if ((bs->inventory[INVENTORY_UMP45] > 0) && (bs->inventory[INVENTORY_UMP45_AMMO] > 25))
	{
		return 70;
	}

	if ((bs->inventory[INVENTORY_SPAS12] > 0) && (bs->inventory[INVENTORY_SPAS12_AMMO] > 6))
	{
		return 60;
	}

	/*if ((bs->inventory[INVENTORY_BENELLI] > 0) && (bs->inventory[INVENTORY_BENELLI_AMMO] > 6))
	{
		return 60;
	}*/

	if ((bs->inventory[INVENTORY_HK69] > 0) && (bs->inventory[INVENTORY_HK69_AMMO] > 4))
	{
		return 80;
	}

	if ((bs->inventory[INVENTORY_AK103] > 0) && (bs->inventory[INVENTORY_AK103_AMMO] > 30))
	{
		return 80;
	}

	if ((bs->inventory[INVENTORY_M4] > 0) && (bs->inventory[INVENTORY_M4_AMMO] > 30))
	{
		return 80;
	}

	if ((bs->inventory[INVENTORY_NEGEV] > 0) && (bs->inventory[INVENTORY_NEGEV_AMMO] > 30))
	{
		return 80;
	}
...
int BotWantsToCamp(bot_state_t *bs)
{
	...
	if (((bs->inventory[INVENTORY_PSG1] <= 0) || (bs->inventory[INVENTORY_PSG1_AMMO] < 6)) &&
		((bs->inventory[INVENTORY_SR8] <= 0) || (bs->inventory[INVENTORY_SR8_AMMO] < 6)) &&
		((bs->inventory[INVENTORY_G36] <= 0) || (bs->inventory[INVENTORY_G36_AMMO] < 30)))
	{
		return qfalse;
	}
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl)
{
	...
	if (movetype == MOVE_JUMP)
	{
		...
		else if (bs->attackjump_time > FloatTime())
		{
			movetype = MOVE_WALK;
		}
		else
	...
	if (UT_WEAPON_GETID(bs->cur_ps.weaponData[bs->cur_ps.weaponslot]) == UT_WP_KNIFE)
	{
		attack_dist  = 0;
		attack_range = 0;
	}
...
void BotAimAtEnemy(bot_state_t *bs)
{
	...
	if (wi.number == UT_WP_BERETTA ||
        wi.number == UT_WP_DEAGLE ||
        wi.number == UT_WP_GLOCK ||
        wi.number == UT_WP_MP5K ||
        wi.number == UT_WP_LR ||
        wi.number == UT_WP_G36 ||
        wi.number == UT_WP_AK103 ||
        wi.number == UT_WP_M4 ||
        wi.number == UT_WP_NEGEV ||
        wi.number == UT_WP_COLT1911 ||
        wi.number == UT_WP_MAC11
        //wi.number == UT_WP_MAGNUM ||
        //wi.number == UT_WP_DUAL_BERETTA ||
        //wi.number == UT_WP_DUAL_DEAGLE ||
        //wi.number == UT_WP_DUAL_GLOCK ||
        //wi.number == UT_WP_DUAL_MAGNUM
        ) {

		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0, 1);
	}
	else if (wi.number == UT_WP_SPAS12 /*|| wi.number == UT_WP_BENELLI*/)
	{
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
	}
	else if (wi.number == UT_WP_GRENADE_HE ||
		 wi.number == UT_WP_GRENADE_FLASH ||
		 wi.number == UT_WP_GRENADE_SMOKE ||
		 wi.number == UT_WP_HK69)
	{
...
//					origin[2] += 1;  //changing aim height
					//
					VectorClear(cmdmove);
				...
				else if (aim_skill > 0.4)
				{
					...
					dir[2] = 0;
					//
					VectorNormalize(dir, speed);
					speed = speed / entinfo.update_time;
...
void BotCheckAttack(bot_state_t *bs)
{
	...
	VectorCopy(bs->origin, start);
	start[2] += bs->cur_ps.viewheight;
	AngleVectorsFR(bs->viewangles, forward, right, NULL);
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal)
{
	...
	if (!strcmp(classname, "func_door"))
	{
		...
		if (*model)
		{
			...
			{
				VectorClear(angles);
				BotModelMinsMaxs(modelindex, ET_MOVER, 0, absmin, absmax);
				//
				numareas = trap_AAS_BBoxAreas(absmin, absmax, areas, MAX_ACTIVATEAREAS * 2);
...
{
	int 		movetype, bspent;
	vec3_t			target, dir, hordir, start, end, mins, maxs, sideward, angles, up = { 0, 0, 1};
	aas_entityinfo_t	entinfo;
	...
	VectorNormalize(hordir, vn);
	if (vn < 0.1)
	{
		VectorSet(angles, 0, 360 * random(), 0);
		AngleVectorsF(angles, hordir, NULL, NULL);
	}
	//
	...
	bot = &g_entities[bs->cur_ps.clientNum];

	VectorSubtract( bot->client->ps.origin, range, mins );
	VectorAdd( bot->client->ps.origin, range, maxs );
	...
	for (i = 0; i < num; i++)
	{
		...
		if(hit->classname)
		{
//			  if(!Q_stricmp(hit->classname, "func_rotating_door"))
			if(hit->classhash==HSH_func_rotating_door)
			{
				if((hit->mover->apos2[YAW] == 0) && (hit->mover->apos1[YAW] == 0) && ((bot->lastDoorTime + 5000) < level.time))
				{
					hit->use(hit, hit, bot);
					bot->client->ps.torsoAnim  = TORSO_USE;
					bot->client->ps.torsoTimer = 1500;
					bot->lastDoorTime	   = level.time;
				}
			}
...
//			  else if(!Q_stricmp(hit->classname, "func_breakable"))
			else if(hit->classhash==HSH_func_breakable)
			{
				target[0] = hit->r.absmin[0] + (hit->r.absmax[0] - hit->r.absmin[0]) / 2;
				target[1] = hit->r.absmin[1] + (hit->r.absmax[1] - hit->r.absmin[1]) / 2;
				target[2] = hit->r.absmin[2] + (hit->r.absmax[2] - hit->r.absmin[2]) / 2;

				//aim at the glass
				VectorSubtract(target, bs->eye, dir);
				vectoangles(dir, bs->ideal_viewangles);
				vectoangles(dir, bot->client->ps.viewangles);
				//set the aim target before trying to attack
				VectorCopy(target, bs->aimtarget);
		...
		case EV_GENERAL_SOUND:
			{
				...
				if (state->number == bs->client)
				{
					...
					{
						// TODO: Any special conditions on falling?
					}
				}
				break;
			}
