...
void UpdateTournamentInfo( void )
{
	...
	else
	{
		...
		Com_sprintf( msg, sizeof(msg), "postgame %i %i %i %i %i %i %i %i",
...
static gentity_t *SpawnModelOnVictoryPad( gentity_t *pad, vec3_t offset, gentity_t *ent, int place )
{
	...
	HASHSTR(body->classname,body->classhash);
	...
	body->s.event		= 0;
	body->s.pos.trType	= TR_INTERPOLATE;
	body->s.groundEntityNum = ENTITYNUM_WORLD;
	body->s.legsAnim	= LEGS_IDLE;
	body->s.torsoAnim	= TORSO_STAND_PISTOL;

	if(body->s.weapon == UT_WP_NONE)
	{
		body->s.weapon = UT_WP_BERETTA;
	}

	if(body->s.weapon == UT_WP_KNIFE)
	{
...
static void CelebrateStop( gentity_t *player )
{
	...
	if(player->s.weapon == UT_WP_KNIFE)
	{
		anim = TORSO_STAND_RIFLE;
	}
	else
	{
		anim = TORSO_STAND_PISTOL;
	}
...
{
	player->s.torsoAnim = ((player->s.torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | TORSO_USE;
	player->nextthink	= level.time + TIMER_GESTURE;
	player->think		= CelebrateStop;

	/*
	player->client->ps.events[player->client->ps.eventSequence & (MAX_PS_EVENTS-1)] = CSE_ENC(EV_TAUNT);
...
static void PodiumPlacementThink( gentity_t *podium )
{
	...
	AngleVectorsF( level.intermission_angle, vec, NULL, NULL );
