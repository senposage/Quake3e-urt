 ...
 * $Author: highsea $
 * $Revision: 1.9 $
 * $Modtime: 1/23/01 7:30a $
 * $Date: 2003/07/06 20:46:13 $
 *
 *****************************************************************************/
...
{
	// BOT CHATTING IS DISABLED FOR NOW
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time)
{
	...
	if (bi->actionflags & ACTION_BANDAGE)
	{
		ucmd->buttons |= BUTTON_BANDAGE;
	}
...
//	if (bi->actionflags & ACTION_FOLLOWME) ucmd->buttons |= BUTTON_FOLLOWME;
	...
	angles[ROLL] = 0;
	AngleVectorsFR(angles, forward, right, NULL);
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time)
{
	...
	// Added to ensure bots figure out semi-automatic
	if(bs->lastucmd.buttons & BUTTON_ATTACK)
	{
		if(bs->cur_ps.stats[STAT_UTPMOVE_FLAGS] & UT_PMF_FIRE_HELD)
		{
			if(bg_weaponlist[utPSGetWeaponID(&bs->cur_ps)].modes[utPSGetWeaponMode(&bs->cur_ps)].flags & UT_WPMODEFLAG_NOHOLD)
			{
				bs->lastucmd.buttons &= (~BUTTON_ATTACK);
			}
		}
	}
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart)
{
	...
	{
		memset(&botstates[client], 0, sizeof(botstates[client]));
	}
	bs = &botstates[client];
...
int BotAISetup( int restart )
{
	...
	// the maximum number of weapons in game
	trap_BotLibVarSet("max_weaponinfo", va("%d", UT_MAX_WEAPONS));
