...
struct BlockInfo {
  ...
  int info;		    /* 31 bits size: 1 bit free boolean */
#define INFO_FREE 1
...
{
  Com_Printf("*** " __FILE__ " Couldn't allocate %zu bytes\n", size);
  print_lists();
}
#else
...
#if 1
  Com_Printf("List of BLOCKS (in address order):\n");
  while(block) {
    Com_Printf("  START %p END %p SIZE %d FLAG %s\n",
	   (char *)block,
	   (char *)block+(block->info&INFO_SIZE), block->info&INFO_SIZE,
	   (block->info&INFO_FREE)?"free":"used");
    block = block->higher;
  }
  Com_Printf("End of BLOCKS:\n");
#endif
  print_sizes();
}
...
void *bmalloc(size_t size)
{
...
#ifdef DEBUG
  {
    ...
    Com_Printf("%d bmalloc(%lu) [%d]", count++, size, realsize);
  }
#endif
  ...
  if(mem) {
    ...
    if( size != (size_t)(block->info&INFO_SIZE)) {
...
#ifdef DEBUG
    Com_Printf(" address(%p) [%p]\n", block,
	    (char*)block+sizeof(struct BlockInfo));
#endif
