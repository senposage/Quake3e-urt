...
void Menus_CloseAll(void)
{
	...
	for (i = 0; i < menuCount; i++)
	{
		if(!Q_stricmp(Menus[i].window.name, "main"))
			trap_Cvar_Set("ui_mainMenu", "0");
...
{
	static char  playing[MAX_QPATH] = "";
	const char	 *val;

	if (String_Parse(args, &val))
	{
		if (Q_stricmp ( playing, val ) != 0)
		{
			DC->stopBackgroundTrack();
			DC->startBackgroundTrack(val, val);

			Com_sprintf ( playing, sizeof(playing), "%s", val );
		}
	}
}
...
qboolean Item_ListBox_HandleKey(itemDef_t *item, int key, qboolean down, qboolean force) {
	...
	if (force || (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) && (item->window.flags & WINDOW_HASFOCUS))) {
		...
		if (item->window.flags & WINDOW_HORIZONTAL) {
			...
			// @Fenix: added K_MWHEELUP
			if ((key == K_UPARROW) || (key == K_KP_UPARROW) || (key == K_MWHEELUP)) {

			    // @Fenix: do not skip elements!
				...
				// @Fenix: play a sound to notice the scrolling
				if (playSound && sfx) DC->startLocalSound( *sfx, CHAN_LOCAL_SOUND );

				return qtrue;

			}

			// @Fenix: added K_MWHEELDOWN
	...
	if ((item->window.flags & WINDOW_HASFOCUS) && item->cvar && Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory))
	{
		if ((key == K_MOUSE1) || (key == K_ENTER) || (key == K_MOUSE2) || (key == K_MOUSE3))
		{
			...
			if (editDef)
			{
				...
				if (item->text && (item->textRect.x || item->textRect.w))
				{
					x = item->textRect.x + item->textRect.w + 8 + item->textalignx;
				}
				else
				{
...
static bind_t		 g_bindings[] = {
	...
	{ "ut_weapmode",		 -1, -1,-1 - 1 },								// Weapon mode
	{ "ut_weapdrop",		 -1, -1, -1, -1 }, // Drop weapon
	{ "+button5",			 -1, -1, -1, -1 }, // Reload
	{ "+button6",			 -1, -1, -1, -1 }, // Bandage
	{ "+button7",			 -1, -1, -1, -1 }, // Use
	{ "+button8",			 -1, -1, -1, -1 }, // Sprint
	{ "ut_itemuse", 		 -1, -1, -1, -1 }, // Use item
	{ "ut_itemnext",		 -1, -1, -1, -1 }, // Next item
	{ "ut_itemprev",		 -1, -1, -1, -1 }, // Previous item
	{ "ut_itemdrop",		 -1, -1, -1, -1 }, // Drop item
	{ "ut_zoomin",			 -1, -1, -1, -1 }, // Zoom weapon in
	{ "ut_zoomout", 		 -1, -1, -1, -1 }, // Zoom weapon out
	{ "ut_zoomreset",		 -1, -1, -1, -1 }, // Reset weapon zoom
	{ "ui_selectteam",		 -1, -1, -1, -1 }, // Select team
	{ "ui_selectgear",		 -1, -1, -1, -1 }, // Select gear
	{ "ui_radio",			 -1, -1, -1, -1 }, // Select radio
	{ "maptoggle",			 -1, -1, -1, -1 }, // Minimap toggle
	{ "kick",			 -1, -1, -1, -1 }, // Kick command
	{ "+button12",			 -1, -1, -1, -1 }, // Pressdown Minimap key
};
...
*/
	...
	for(i = 0; i < 3; i++)
	{
		if (modelPtr->rotationSpeed[i])
		{
			if (DC->realTime > modelPtr->rotationTime[i])
			{
				modelPtr->rotationTime[i] = DC->realTime + modelPtr->rotationSpeed[i];
				modelPtr->angle[i]	  = (int)(modelPtr->angle[i] + modelPtr->rotationDir[i]) % 360;
			}
		}
	}

	{
		...
		{
			xr = (float)((float)(modelPtr->rotationTime[0] - DC->realTime) / (float)modelPtr->rotationSpeed[0]);
		}

		if(modelPtr->rotationSpeed[1])
		{
			yr = (float)((float)(modelPtr->rotationTime[1] - DC->realTime) / (float)modelPtr->rotationSpeed[1]);
		}

		if(modelPtr->rotationSpeed[2])
		{
			zr = (float)((float)(modelPtr->rotationTime[2] - DC->realTime) / (float)modelPtr->rotationSpeed[2]);
		}

		VectorSet( angles,
			   (float)modelPtr->angle[0] - (modelPtr->rotationDir[0] * xr * 1.0f),
			   (float)modelPtr->angle[1] - (modelPtr->rotationDir[1] * yr * 1.0f),
			   (float)modelPtr->angle[2] - (modelPtr->rotationDir[2] * zr * 1.0f));
	}

	AnglesToAxis( angles, ent.axis );

	VectorScale ( ent.axis[0], modelPtr->scale[0], ent.axis[0] );
	VectorScale ( ent.axis[1], modelPtr->scale[1], ent.axis[1] );
	VectorScale ( ent.axis[2], modelPtr->scale[2], ent.axis[2] );

	ent.hModel	   = item->asset;
	ent.customSkin = modelPtr->skin;
			...
			for (i = listPtr->startPos; i < count; i++)
			{
				...
				else
				{
					...
					else if (text)
					{
						// DensitY
						/*
							Gay hack for Democasting, since we want the text to show up are Yellow if its supports
							Democasting..
						*/
						if(item->special == (float)FEEDER_DEMOS)
						{
							if(DC->DemoCaster[(int)( i )])
							{
								DemoColor[0] = DemoColor[1] = DemoColor[3] = 1.0f;
								DemoColor[2] = 0.0f;
							}
							else
							{
								DemoColor[0] = DemoColor[1] = DemoColor[2] = DemoColor[3] = 1.0f;
							}
						}
						else
						{
							DemoColor[0] = item->window.foreColor[0];
							DemoColor[1] = item->window.foreColor[1];
							DemoColor[2] = item->window.foreColor[2];
							DemoColor[3] = item->window.foreColor[3];
						}

						DC->drawText(x + 4, y + listPtr->elementHeight, item->textscale, DemoColor, text, 0, 0, item->textStyle);
					}
				}
	...
	if (DC->ownerDrawItem)
	{
		...
		if (item->text)
		{
			...
			if (item->text[0])
			{
				...
				DC->ownerDrawItem(item->textRect.x + item->textRect.w + 8, item->window.rect.y, item->window.rect.w, item->window.rect.h, 0, item->textaligny, item->window.ownerDraw, item->window.ownerDrawFlags, item->window.ownerDrawParam, item->window.ownerDrawParam2, item->alignment, item->special, item->textscale, color, item->window.background, item->textStyle );
			}
			else
			{
				DC->ownerDrawItem(item->textRect.x + item->textRect.w, item->window.rect.y, item->window.rect.w, item->window.rect.h, 0, item->textaligny, item->window.ownerDraw, item->window.ownerDrawFlags, item->window.ownerDrawParam, item->window.ownerDrawParam2, item->alignment, item->special, item->textscale, color, item->window.background, item->textStyle );
			}
		}
		else
		{
			DC->ownerDrawItem(item->window.rect.x, item->window.rect.y, item->window.rect.w, item->window.rect.h, item->textalignx, item->textaligny, item->window.ownerDraw, item->window.ownerDrawFlags, item->window.ownerDrawParam, item->window.ownerDrawParam2, item->alignment, item->special, item->textscale, color, item->window.background, item->textStyle );
		}
	}
}
...
void Item_Paint(itemDef_t *item)
{
...

	{
		...
		rectText = item->textRect;

		if (DC->getRectOffset)
		{
			DC->getRectOffset ( item->window.rectOffset, &item->window.rect );
			DC->getRectOffset ( item->window.rectOffset, &item->textRect );
		}
		...
		item->window.rect = rect;
		item->textRect	  = rectText;
	}
}
...
qboolean ItemParse_model_rotation( itemDef_t *item, int handle )
{
	...
	if (PC_Int_Parse(handle, &modelPtr->rotationSpeed[0]))
	{
		if (PC_Int_Parse(handle, &modelPtr->rotationSpeed[1]))
		{
			if (PC_Int_Parse(handle, &modelPtr->rotationSpeed[2]))
			{
				modelPtr->rotationDir[0]   = modelPtr->rotationSpeed[0] < 0 ? -1 : 1;
				modelPtr->rotationDir[1]   = modelPtr->rotationSpeed[1] < 0 ? -1 : 1;
				modelPtr->rotationDir[2]   = modelPtr->rotationSpeed[2] < 0 ? -1 : 1;

				modelPtr->rotationSpeed[0] = abs(modelPtr->rotationSpeed[0]);
				modelPtr->rotationSpeed[1] = abs(modelPtr->rotationSpeed[1]);
				modelPtr->rotationSpeed[2] = abs(modelPtr->rotationSpeed[2]);
...
qboolean ItemParse_model_angle( itemDef_t *item, int handle )
{
	...
	if (PC_Int_Parse(handle, &modelPtr->angle[0]))
	{
		if (PC_Int_Parse(handle, &modelPtr->angle[1]))
		{
			if (PC_Int_Parse(handle, &modelPtr->angle[2]))
			{
				return qtrue;
			}
		}
...
// model_scale <integer>
qboolean ItemParse_model_scale( itemDef_t *item, int handle )
{
	modelDef_t	*modelPtr;
	...
	if (PC_Float_Parse(handle, &modelPtr->scale[0]))
	{
		if (PC_Float_Parse(handle, &modelPtr->scale[1]))
		{
			if (PC_Float_Parse(handle, &modelPtr->scale[2]))
			{
				return qtrue;
			}
		}
...
// model_skin <filename>
qboolean ItemParse_model_skin( itemDef_t *item, int handle )
{
	modelDef_t	*modelPtr;
	const char	*temp;
	...
	modelPtr->skin = DC->registerSkin(temp);
...
// rect <rectangle>
qboolean ItemParse_rect_offset( itemDef_t *item, int handle )
{
	if (!PC_Int_Parse(handle, &item->window.rectOffset))
	{
		return qfalse;
	}
	return qtrue;
 ...
 * $(function)
 */
qboolean ItemParse_ownerdrawParam2( itemDef_t *item, int handle )
{
	int  i;
...
keywordHash_t  itemParseKeywords[] = {
	...
	{ "model_scale",	  ItemParse_model_scale,	  NULL },
	{ "model_skin", 	  ItemParse_model_skin,   NULL },
	{ "rect",		  ItemParse_rect,		  NULL },
	{ "rectoffset", 	  ItemParse_rect_offset,	  NULL },
	...
	{ "ownerdrawParam",   ItemParse_ownerdrawParam,   NULL },
	{ "ownerdrawParam2",  ItemParse_ownerdrawParam2,  NULL },
 ...
 * $(function)
 */
qboolean MenuParse_ownerdrawParam( itemDef_t *item, int handle )
{
	int    i;
	menuDef_t  *menu = (menuDef_t *)item;
 ...
 * $(function)
 */
qboolean MenuParse_ownerdrawParam2( itemDef_t *item, int handle )
{
	int    i;
	menuDef_t  *menu = (menuDef_t *)item;
...
keywordHash_t  menuParseKeywords[] = {
	...
	{ "ownerdrawParam",   MenuParse_ownerdrawParam,  NULL },
	{ "ownerdrawParam2",  MenuParse_ownerdrawParam2, NULL },
 ...
 * $(function)
 */
crosshair_t *utCrosshairGetAt ( int index )
{
	if (index >= crosshairCount)
	{
		return &Crosshairs[0];
	}
 ...
 * $(function)
 */
int utCrosshairGetCount ( void )
{
	return crosshairCount;
}

/**
 * $(function)
 */
//static void utCrosshairParse( char *buf )
//{
//}
 ...
 * $(function)
 */
static void utCrosshairLoadAllFromFile ( char *filename )
{
	int 	handle;
	pc_token_t	token;
	...
	while (1)
	{
		...
		// HAs to start with a open sqiggy
		...
		// Make sure we dont have too many
		if (crosshairCount == MAX_CROSSHAIRS)
		{
		...
		while (1)
		{
			...
			// If this is the end then go on to the next one
			if (!strcmp( token.string, "}" ))
			{
				crosshairCount++;
				break;
			...
			// Get its value, if it has none then skip it
			if (!trap_PC_ReadToken ( handle, &token ))
			{
				continue;
			}
			...
			{
				Crosshairs[crosshairCount].name = bmalloc ( strlen ( token.string ) + 1 );
				strcpy ( Crosshairs[crosshairCount].name, token.string );
			}
			else if (!Q_stricmp ( key, "center" ))
			{
				Crosshairs[crosshairCount].center = DC->registerShaderNoMip(token.string);
			}
			else if (!Q_stricmp ( key, "right" ))
			{
				Crosshairs[crosshairCount].right = DC->registerShaderNoMip(token.string);
			}
			else if (!Q_stricmp ( key, "left" ))
			{
				Crosshairs[crosshairCount].left = DC->registerShaderNoMip(token.string);
			}
			else if (!Q_stricmp ( key, "top" ))
			{
				Crosshairs[crosshairCount].top = DC->registerShaderNoMip(token.string);
			}
			else if (!Q_stricmp ( key, "bottom" ))
			{
				Crosshairs[crosshairCount].bottom = DC->registerShaderNoMip(token.string);
			}
		}
	}

	// New sharp corsshairs..
	if (crosshairCount != 8)
	{
		trap_Error( "8 crosshairs expected.\n" );
	}
...
/*
	...
	numdirs = trap_FS_GetFileList("scripts", ".crosshair", dirlist, 1024 );
	dirptr	= dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		...
		utCrosshairLoadAllFromFile ( filename );
	}
*/

	utCrosshairLoadAllFromFile ( "scripts/urbanterror.crosshair" );

	Com_Printf( va( "%i crosshairs parsed\n", crosshairCount ));
}
