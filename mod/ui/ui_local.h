...
typedef struct {
	...
	/*
	There are problems with shorts in structures with the qvm code
	I don't know if it's q3asm, or the JIT on x86_64, but if we are using a short here,
	The netadr_t instances become somehow readonly and hold random data values.

	We still need a structure that's compatible with the engine's version,
	so doubling up with bytes and doing manual casts to access the data.
	unsigned short	port;
	*/
	byte port[2];
...
extern vmCvar_t  ui_browserSortDir;
extern vmCvar_t  ui_browserShowFull;
extern vmCvar_t  ui_browserShowEmpty;
extern vmCvar_t  ui_browserShowAccounts; //@Barbatos
extern vmCvar_t  ui_browserShowPrivate; //@Barbatos
...
extern vmCvar_t  ui_modversion; //@Barbatos

// URban terror
extern vmCvar_t  ui_gearItemCount;
...
extern char 	*ui_keyboardInterfaceOptions[5];
extern int		currStopID;
...
#define MAX_MAPS				512 //@Barbatos 09/05/2012 was 128
...
typedef struct
{
	...
	int 				 playerClientNum[MAX_CLIENTS];
	...
	int 				 q3HeadCount[TEAM_NUM_TEAMS];
	char				 q3HeadNames[TEAM_NUM_TEAMS][MAX_PLAYERMODELS][64];
	qhandle_t			 q3HeadIcons[TEAM_NUM_TEAMS][MAX_PLAYERMODELS];
	int 				 q3SelectedHead;
...
// Urban terror
	...
	int   gearSelect [GEAR_MAX];
	int   gearActive;
	int   gearItemCount;

	int   statsplayerIndex;
	int   statsweaponsIndex;

	char  StatsCommand[256];
	int   safeUpdate;
}	uiInfo_t;
...
extern int		UI_Text_Width(const char *text, float scale, int limit);
extern int		UI_Text_Height(const char *text, float scale, int limit);
extern void 	UI_Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style);
extern void 	UI_Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int limit, int style);
extern void 	UI_Text_Paint_Limit(float *maxX, float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit);
