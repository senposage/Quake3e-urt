...
tryagain:
	...
	if(pi->weaponModel == 0)
	{
		if(weaponNum == UT_WP_BERETTA)
		{
			weaponNum = UT_WP_NONE;
			goto tryagain;
		}
		weaponNum = UT_WP_BERETTA;
		goto tryagain;
	}

	if ((weaponNum == UT_WP_BERETTA) || (weaponNum == UT_WP_KNIFE) || (weaponNum == UT_WP_SR8))
	{
...
static void UI_ForceTorsoAnim( playerInfo_t *pi, int anim )
{
	...
	if ((anim == TORSO_ATTACK_PISTOL) || (anim == TORSO_ATTACK_RIFLE) || (anim == TORSO_ATTACK_KNIFE))
	{
...
static void UI_TorsoSequencing( playerInfo_t *pi )
{
	...
	if(currentAnim == TORSO_USE)
	{
		UI_SetTorsoAnim( pi, TORSO_STAND_RIFLE );
		return;
	}

	if((currentAnim == TORSO_ATTACK_PISTOL) || (currentAnim == TORSO_ATTACK_RIFLE))
	{
		UI_SetTorsoAnim( pi, TORSO_STAND_RIFLE );
		return;
	}
...
static float UI_MovedirAdjustment( playerInfo_t *pi )
{
	...
	AngleVectorsF( relativeAngles, moveVector, NULL, NULL );
...
static void UI_PlayerAngles( playerInfo_t *pi, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] )
{
	...
	if (((pi->legsAnim & ~ANIM_TOGGLEBIT) != LEGS_IDLE) || ((pi->torsoAnim & ~ANIM_TOGGLEBIT) != TORSO_STAND_PISTOL)) {
...
float	UI_MachinegunSpinAngle( playerInfo_t *pi )
{
	...
	if(torsoAnim == TORSO_ATTACK_RIFLE)
	{
		torsoAnim = TORSO_ATTACK_RIFLE;
	}

	if (pi->barrelSpinning == !(torsoAnim == TORSO_ATTACK_PISTOL))
	{
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time )
{
	...
	// add the gun
	//
	if (pi->currentWeapon != UT_WP_NONE)
	{
	...
	//
	if ((pi->realWeapon == UT_WP_BERETTA) || (pi->realWeapon == UT_WP_KNIFE) || (pi->realWeapon == UT_WP_SR8))
	{
		vec3_t	angles;
		...
		if((pi->realWeapon == UT_WP_KNIFE) || (pi->realWeapon == UT_WP_SR8))
		{
...
//	}
	...
	{
		Com_sprintf( filename, sizeof(filename), "models/players/heads/%s/head_%s.skin", modelName, skinName );
		pi->headSkin = trap_R_RegisterSkin( filename );
	}
...
======================
*/
static qboolean UI_ParseAnimationFile ( char *filename, animation_t *animations )
{
	pc_token_t  token;
	int	    anim;
	int	    handle;
...
static qboolean UI_ParseAnimationFile( const char *filename, animation_t *animations ) {
	...
	while ( 1 ) {
		...
		if ( !Q_stricmp( token, "footsteps" ) ) {
			...
			continue;
		} else if ( !Q_stricmp( token, "utmodel" ) ) {
			continue;
		}
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName, const char *headName, const char *teamName )
{
	...
	// If its not one of our models then default it
/*	if ( Q_stricmp ( modelName, "athena" ) && Q_stricmp ( modelName, "orion" ) )
	{
		trap_Cvar_Set ( "model", bg_teamSkins[TEAM_FREE][0] );
		trap_Cvar_Set ( "team_model_red", bg_teamSkins[TEAM_RED][0] );
		trap_Cvar_Set ( "team_model_blue", bg_teamSkins[TEAM_BLUE][0] );

		strcpy ( modelName, "orion" );
		strcpy ( skinName, "desert_w" );
	} */
...
===============
*/
void UI_PlayerInfo_SetModel( playerInfo_t *pi, const char *model, const char *headmodel, char *teamName, int weapon )
{
...
void UI_PlayerInfo_SetInfo( playerInfo_t *pi, int legsAnim, int torsoAnim, vec3_t viewAngles, vec3_t moveAngles, weapon_t weaponNumber, qboolean chat )
{
	...
	if (pi->newModel)
	{
		...
		pi->legs.animationNumber = LEGS_IDLE;
	...
	else if (weaponNumber != UT_WP_NONE)
	{
	...
	if ((torsoAnim == BOTH_DEATH_CHEST) || (legsAnim == BOTH_DEATH_CHEST))
	{
		...
		pi->weapon = pi->currentWeapon = UT_WP_NONE;
		UI_PlayerInfo_SetWeapon( pi, pi->weapon );
	...
	switch (bg_weaponlist[pi->weapon].type)
	{
		default:
		...
		case UT_WPTYPE_SMALL_GUN_DUAL:
			torsoAnim = TORSO_STAND_DUAL;
			break;

		case UT_WPTYPE_MEDIUM_GUN:

		    if (bg_weaponlist[pi->weapon].q3weaponid == UT_WP_MAC11) {
		        torsoAnim = TORSO_STAND_PISTOL;
		    } else {
	...
	else if ((currentAnim == TORSO_USE || currentAnim == TORSO_ATTACK_RIFLE) && (torsoAnim != currentAnim))
	{
		pi->pendingTorsoAnim = torsoAnim;
	}
